...
HaulCount <- HaulFullClean %>%
  group_by(system,year,month) %>%
  summarise(days = (n_distinct(Sampling_Date))) %>%
  ungroup()

HaulInfo <- HaulCount %>%
  group_by(system) %>%
  summarise(minDays = min(days), 
            maxDays = max(days),
            avgDays = mean(days))

TestHydro <- TidyHydro %>%
  select(c('Reference', 'Sampling_Date'))

testttt  <- HaulFullClean %>%
  select(c('Reference')) %>%
  inner_join(TestHydro)
