...
#| fig-width: 6.5
#| fig-height: 5
#| fig-cap: "Partitioned betadiversity calculated pairwise with respect to the first year of available data. Trend lines calculated using seasonal ANCOVA models."
...
sigSummaryPercent <- sigSummary %>%
  select(system, season, count, sigCount, pos_percent, neg_percent) %>%
  rename(posPercent = pos_percent) %>%
  rename(negPercent = neg_percent)
...
The majority of taxa did not exhibit significant changes in abundance over time, with a large group centered around zero in both summer and winter ([@fig-winnersLosers]). However, the percentage of taxa changing negatively was overall greater in the summer than the winter (*t* = `r format(negSig$statistic, digits = 3)`, *p* = `r scales::pvalue(negSig$p.value)`; see [@tbl-sigTaxa]). The majority of thse taxa were either tropical/subtropical-associated or tropical/subtropical/temperate-associated ([@tbl-ecoSlopes]).
...
The majority of taxa that had statistically significant slopes still experienced rather modest changes in abundance, while a few taxa had rates of change an order of magnitude greater than the majority ([@fig-sigSlopes]).
