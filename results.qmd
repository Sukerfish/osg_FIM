...
diffTaxa <- read.csv(file = "conceptFigs/diffIDTaxa_fars.csv") %>%
...
Throughout the 23-year study period, submerged aquatic vegetation (SAV) displayed nuanced dynamics, with no observed change in Apalachicola Bay for either season, a decrease in Cedar Key during summer only, an increase in Tampa Bay during winter only, and increases observed in Charlotte Harbor (both seasons; [@fig-BVCPlot]). Water temperature increased during summer and winter for all estuary systems, but the rate in winter was more than triple that observed during the summer ([@fig-TempPlot]). Of the `r ncol(SXS_filtered_spp)` total taxa examined, `r length(unique(sigSlopes$Scientificname))` (`r paste0(round(((length(unique(sigSlopes$Scientificname)))/(ncol(SXS_filtered_spp)))*100, 0), "%")`) changed significantly in abundance over time across all systems, nearly equally balanced between increases and decreases in abundances.
...
The dynamics of SAV differed across systems within each season (Year:System interaction term for summer *p* = `r scales::pvalue(as.data.frame(summerBVCSummaryANC$p.value)[3,])` and winter *p* = `r scales::pvalue(as.data.frame(winterBVCSummaryANC$p.value)[3,])`). Therefore, I used independent models for each system and season combination. The northernmost system in this study, Apalachicola Bay, did not have any significant change in SAV over the sampling period for either season (@fig-BVCPlot; @tbl-bvcAncova). Significant changes in SAV coverage occurred in Cedar Key (decreased in summer; *p* \<0.05; @tbl-bvcAncova), in Tampa Bay (increased in winter; *p* \<0.05; @tbl-bvcAncova), and in Charlotte Harbor (increased for both seasons; *p* \<0.05; @tbl-bvcAncova). In addition, mean annual percent SAV differed across all systems within each season with lower coverage in northern systems than southern ones for both seasons (*p* \<0.05 for all pairwise comparisons; @tbl-bvcMeans; see also @tbl-bvcMeanContrasts).
...
During the 23-year study period, water temperature increased consistently across all systems (Year:System interaction term for summer *p* = `r scales::pvalue(as.data.frame(summerSummaryANC$p.value)[3,])` and winter *p* = `r scales::pvalue(as.data.frame(winterSummaryANC$p.value)[3,])`). Summer water temperature increased at a rate of `r format(as.data.frame(summerFinalTidy$estimate)[5,], digits = 2)`°Cyr^-1^ ± `r format(summerFinalTidy[5,]$std.error, digits = 2)` and winter water temperature increased more than three times as quickly at a rate of `r format(as.data.frame(winterFinalTidy$estimate)[5,], digits = 2)`°Cyr^-1^ ± `r format(winterFinalTidy[5,]$std.error, digits = 2)` (@fig-TempPlot). Despite the consistency in the rates of increase, post-hoc contrasts revealed the long-term mean water temperatures differed among most systems, with Charlotte Harbor consistently warmer than all other estuaries in both seasons (*p* \<0.05 for 9 out of 12 pairwise comparisons; @tbl-tempMeans).
...
Total abundance of fauna was generally higher (mean total abundance `r format(abundStats[1,]$meanAb, digits = 2)` ±`r format(abundStats[1,]$seAb, digits = 2)`) for all systems in the summer season compared to winter (mean total abundance `r format(abundStats[2,]$meanAb, digits = 2)` ±`r format(abundStats[2,]$seAb, digits = 2)`; [@fig-AbundPlot]). The Year:System interaction terms were significant in the GLMMs for abundance (*p* \<0.05 for both seasons), therefore, I separated the models for each system and season combination. Within these independent GLMMs, total abundance decreased for Apalachicola Bay in winter (*p* = `r scales::pvalue(abundOutTidy[18,]$p.value)`; @tbl-abundModels), increased for Tampa Bay in summer (*p* = `r scales::pvalue(abundOutTidy[14,]$p.value)`; @tbl-abundModels), and remained stable for all other systems ([@tbl-abundModels]). Summer abundances were positively related to SAV coverage for all systems except Cedar Key (*p* = `r scales::pvalue(min(pValuesBvc[pValuesBvc$Season=="Summer",]$p.value))` to `r scales::pvalue(max(pValuesBvc[pValuesBvc$Season=="Summer",]$p.value))`; @tbl-abundModels). Further, I did not detect a relationship between summer abundances and water temperature (*p* = `r scales::pvalue(min(pValuesTemp[pValuesTemp$Season=="Summer",]$p.value))` to `r scales::pvalue(max(pValuesTemp[pValuesTemp$Season=="Summer",]$p.value))`; @tbl-abundModels). The relationship between abundance and SAV coverage was more nuanced in the winter than during the summer. Winter abundance was negatively related to SAV coverage in Apalachicola Bay (*p* = `r scales::pvalue(pValuesBvc[pValuesBvc$Season=="Summer"&pValuesBvc$System=="AP",]$p.value)`; @tbl-abundModels) and was not related with SAV coverage in Cedar Key (*p* \>0.05; @tbl-abundModels). However, in Tampa Bay and Charlotte Harbor, winter abundance was positively related to SAV coverage (*p* \<0.05; @tbl-abundModels). In addition, winter abundances were positively related to water temperature for all systems except Cedar Key (*p* = `r scales::pvalue(min(pValuesTemp[pValuesTemp$Season=="Winter",]$p.value))` to `r scales::pvalue(max(pValuesTemp[pValuesTemp$Season=="Winter",]$p.value))`; @tbl-abundModels).
...
Mean richness per haul in summer (`r format(nStats[1,]$meanN, digits = 2)` ±`r format(nStats[1,]$seN, digits = 2)`) was consistently higher for each system than in the winter (`r format(nStats[2,]$meanN, digits = 2)` ±`r format(nStats[2,]$seN, digits = 2)`; [@fig-NPlot]). The Year:System interaction terms for the richness GLMMs were not significant in either seasonal model (*p* = `r scales::pvalue(min(richFullInteraction$p.value))` to `r scales::pvalue(max(richFullInteraction$p.value))`), therefore, I used additive models for each system. The revised GLMMs revealed that richness did not change during either summer or winter for any system (summer *p* = `r scales::pvalue(richFinalTidy[richFinalTidy$term=="smallYear"&richFinalTidy$Season=="Summer",]$p.value)` and winter *p* = `r scales::pvalue(richFinalTidy[richFinalTidy$term=="smallYear"&richFinalTidy$Season=="Winter",]$p.value)`; @tbl-richGLMMs). Further, richness in summer was positively related to SAV coverage for all systems (*p* = `r scales::pvalue(min(richFinalTidy[richFinalTidy$term=="bvc_Z"&richFinalTidy$Season=="Summer",]$p.value))`; @tbl-richGLMMs) and not related to water temperature (*p* = `r scales::pvalue(min(richFinalTidy[richFinalTidy$term=="temp_Z"&richFinalTidy$Season=="Summer",]$p.value))`; @tbl-richGLMMs). In contrast, richness in winter was positively related to both SAV coverage (*p* = `r scales::pvalue(min(richFinalTidy[richFinalTidy$term=="bvc_Z"&richFinalTidy$Season=="Summer",]$p.value))`; @tbl-richGLMMs) and water temperature for all systems (*p* = `r scales::pvalue(min(richFinalTidy[richFinalTidy$term=="temp_Z"&richFinalTidy$Season=="Winter",]$p.value))`; @tbl-richGLMMs).
...
Bray-Curtis dissimilarity of species composition increased consistently across all systems within each season (Year:System interaction term for summer *p* = `r scales::pvalue(as.data.frame(betaSummerSummaryANC$p.value)[3,])` and winter *p* = `r scales::pvalue(as.data.frame(betaWinterSummaryANC$p.value)[3,])`). The rate of increase in dissimilarity during summer was `r format(as.data.frame(betaSummerSummaryANC_final$estimate)[2,], digits = 1)`yr^-1^ ± `r format(betaSummerSummaryANC_final[2,]$std.error, digits = 1)`, with winter increasing at twice the summer rate (`r format(as.data.frame(betaWinterSummaryANC_final$estimate)[2,], digits = 1)`yr^-1^ ± `r format(betaWinterSummaryANC_final[2,]$std.error, digits = 1)`; @tbl-betaTrends; @fig-betadiversity). In summer, the variability of dissimilarity was lower than in the winter for all systems except Charlotte Harbor (@tbl-betaCVs). Within dissimilarity overall, there were clear variations in the underlying components of beta diversity (turnover and nestedness), though they were not modeled here.
...
beta_cv_ft <- overallBeta %>%
  mutate(check = ifelse(as.numeric(seasonYear) > 2010, "post-2010", "pre-2010")) %>%
  #relocate(season, .before = system) %>%
  ...
  mutate(ltm = mean(value),
         dev = sd(value),
         cv  = (dev/ltm)*100) %>%
  #distinct(season, system, check, cv) %>%
  distinct(season, system, cv) %>%
  #pivot_wider(values_from = cv, names_from = check) %>%
  arrange(season, system) %>%
  flextable(.) %>%
  merge_v(j = ~ season) %>%
  merge_v(j = ~ system) %>%
  #colformat_double(j = c("estimate", "std.error", "p.value"), digits = 3) %>%
  colformat_double(j = c(3), digits = 2) %>%
  #set_formatter(p.value = function(x) scales::pvalue(x)) %>%
...
load("./Outputs/RDAsforAll_Y_fars.RData")
...
PERMANOVAs indicated assemblage composition and abundance varied differently over time among systems (Year:System interaction term for summer *p* = `r scales::pvalue(summerdf$p.value[3])` and winter *p* = `r scales::pvalue(winterdf$p.value[3])`; [@tbl-fullPERMANOVA]). I therefore conducted separate PEMANOVAs and CAP ordinations for each system within each season [*sensu* @Peake2022]. From the individual PERMANOVA models, assemblage composition changed significantly over time in each estuary and each season (all *p* = 0.001; @tbl-allPERMANOVAs).
...
#| echo: FALSE
#| message: FALSE
load("./Outputs/PERMSforALLDF_factor_fars.RData")
...
load("./Outputs/CAPsforAll_Z_infm_fars.RData")
...
CAP ordinations revealed that in most systems and seasons, there was a slow drift from the earlier years of assemblage composition to the more recent years, as evidenced by the minimal overlap among earlier and recent year centroids ([@fig-centroidCAP]). However, notable exceptions included Cedar Key in the summer, which had separation into two groups transitioning around the year 2010, and Charlotte Harbor, which, for both seasons, had considerably more overlap among earlier and recent year centroids ([@fig-centroidCAP]). In terms of individual taxa identified by the CAPs, in winter, *Leiostomous xanthurus* was consistently identified as the taxa most associated with yearly assemblage differences and was most associated with the centroids of earlier year groups in the dataset ([@tbl-CAPtaxa]; [@fig-centroidCAP]). For the apparent separation in Cedar Key summer, the taxa most aligned with these opposing groups were *L. xanthurus* and *Eucinostomus* spp. ([@fig-centroidCAP]). The years pre-2010 were most aligned with *L. xanthurus* and the more recent years with *Eucinostomus* fishes. Overall, the CAP models explained `r paste0(round(min(topSpp$percent),2), "%")` to `r paste0(round(max(topSpp$percent),2), "%")` of the variability, indicating a large portion was not explained by the assemblage composition and abundances alone. Looking at the assemblage most associated with yearly differences, only five taxa were selected as the top associated with yearly separation on the first two canonical axes for all system and season combinations ([@tbl-CAPtaxa]). Specifically, each system in summer had a different taxon associated with the greatest yearly separation, but in the winter, all systems are best separated on a yearly basis by *L. xanthurus* ([@tbl-CAPtaxa]).
...
SAV and water temperature were significantly related to beta diversity across all systems and seasons ([@fig-RDAforAll]). However, there were no clear temporal patterns in most of the ordinations ([@fig-RDAforAll]). Across systems, Tampa Bay had the most tightly clustered group of yearly centroids, particularly in winter ([@fig-RDAforAll]). In all systems, both SAV coverage and water temperature had strong correlations with the first canonical axis as indicated by their direction and magnitude. Further, the relative magnitude of SAV coverage was generally greater than water temperature in all cases, suggesting a stronger influence on assemblage variability among years.
...
sigSummaryPercent <- sigSummary %>%
  ...
  mutate(sigPercent = round(sigPercent, 3)) %>%
  #mutate(posPercent = round(pos_percent, 3)) %>%
  #mutate(negPercent = round(neg_percent, 3)) %>%
...
The majority of taxa (`r paste0(round((1-(length(unique(sigSlopes$Scientificname)))/(ncol(SXS_filtered_spp)))*100, 0), "%")`) did not exhibit significant changes in abundance over time (`r (ncol(SXS_filtered_spp) - length(unique(sigSlopes$Scientificname)))` out of `r ncol(SXS_filtered_spp)` taxa), with a large group centered around zero rate of abundance change in both summer and winter for all systems ([@fig-winnersLosers]). There was no difference in overall percentage of taxa changing between systems (*t* = `r format(sigSigSys_tidy[1,]$statistic, digits = 2, nsmall = 2)`; *p* = `r scales::pvalue(sigSigSys_tidy[1,]$p.value)`) or seasons (*t* = `r format(sigSig$statistic, digits = 3, nsmall =2)`; *p* = `r scales::pvalue(sigSig$p.value)`). However, the percentage of taxa undergoing population declines was greater in the summer than in the winter (*t* = `r format(negSig$statistic, digits = 3)`; *p* = `r scales::pvalue(negSig$p.value)`; [@tbl-sigTaxa]), with no difference between seasons in the percentage of taxa changing positively (*t* = `r format(posSig$statistic, digits = 3)`; *p* = `r scales::pvalue(posSig$p.value)`; [@tbl-sigTaxa]).
...
Overall, most taxa that had significant changes in abundance were either tropical-associated or ubiquitous in distribution and experienced rather modest changes, yet, a few taxa had rates of change an order of magnitude greater than the rest ([@tbl-ecoSlopes]; [@fig-sigSlopes]).
