...
## Faunal Dynamics {#sec-community-data}
...
Total abundance of fauna was generally higher (mean total abundance `r format(abundStats[1,]$meanAb, digits = 2)` ±`r format(abundStats[1,]$seAb, digits = 2)`) for all systems in the summer season compared to winter (mean total abundance `r format(abundStats[2,]$meanAb, digits = 2)` ±`r format(abundStats[2,]$seAb, digits = 2)`; [@fig-AbundPlot]). The global abundance GLMM had multiple significant time:system interaction terms, therefore, I separated each system and season into independent GLMMs. Total abundance decreased for Apalachicola Bay in winter (*p* = `r scales::pvalue(abundOutTidy[18,]$p.value)`), increased for Tampa Bay in summer (*p* = `r scales::pvalue(abundOutTidy[14,]$p.value)`), and remained stable for all other systems [@tbl-abundModels].
...
# winterNPred <- predict(glmmOutRW)
# summerNPred <- predict(glmmOutRS)
...
## Assemblage structure: patterns and dynamics {#sec-constrained-analyses}
