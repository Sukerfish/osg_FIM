...
The dynamics of SAV differed across systems within each season (time:system interaction term for summer *p* = `r scales::pvalue(as.data.frame(summerBVCSummaryANC$p.value)[3,])` and winter *p* = `r scales::pvalue(as.data.frame(winterBVCSummaryANC$p.value)[3,])`). As a result, I used independent models for each system and season combination. SAV coverage in Charlotte Harbor increased for both seasons, while Tampa Bay increased in winter only, Cedar Key decreased in summer only, and the northernmost estuary in this study, Apalachicola Bay, did not have any significant change in SAV over the sampling period (@fig-BVCPlot; @tbl-bvcAncova). In addition, mean annual percent SAV differed across all systems within each season wherein northern systems averaged less SAV than southern systems in both seasons (*p* \< 0.05 for all pairwise comparisons; @tbl-bvcMeans; see also @tbl-bvcMeanContrasts).

```{r}
#| label: tbl-bvcAncova 
#| echo: FALSE 
#| message: FALSE 
#| tbl-cap: "Coefficients of SAV slope from SAV linear models." 
...
Total abundance of fauna was generally higher (mean total abundance `r format(abundStats[1,]$meanAb, digits = 2)` ±`r format(abundStats[1,]$seAb, digits = 2)`) for all systems in the summer season compared to winter (mean total abundance `r format(abundStats[2,]$meanAb, digits = 2)` ±`r format(abundStats[2,]$seAb, digits = 2)`; [@fig-AbundPlot]). The global abundance GLMMs had multiple significant time:system interaction terms for each season, therefore, I separated each system and season into independent GLMMs. Total abundance decreased for Apalachicola Bay in winter (*p* = `r scales::pvalue(abundOutTidy[18,]$p.value)`), increased for Tampa Bay in summer (*p* = `r scales::pvalue(abundOutTidy[14,]$p.value)`), and remained stable for all other systems [@tbl-abundModels]. Outputs of GLMMs revealed summer abundances were not related to water temperature (*p* = `r scales::pvalue(min(pValuesTemp[pValuesTemp$Season=="Summer",]$p.value))` - `r scales::pvalue(max(pValuesTemp[pValuesTemp$Season=="Summer",]$p.value))`; @tbl-abundModels) and positively related to SAV coverage for all systems except Cedar Key (*p* = `r scales::pvalue(min(pValuesBvc[pValuesBvc$Season=="Summer",]$p.value))` - `r scales::pvalue(max(pValuesBvc[pValuesBvc$Season=="Summer",]$p.value))`; @tbl-abundModels). In contrast, winter total abundance was positively related to water temperature for all systems except Cedar Key (*p* = `r scales::pvalue(min(pValuesTemp[pValuesTemp$Season=="Winter",]$p.value))` - `r scales::pvalue(max(pValuesTemp[pValuesTemp$Season=="Winter",]$p.value))`; @tbl-abundModels) but, as during summer, positively related to SAV coverage for all systems except Cedar Key (*p* = `r scales::pvalue(min(pValuesBvc[pValuesBvc$Season=="Winter",]$p.value))` - `r scales::pvalue(max(pValuesBvc[pValuesBvc$Season=="Winter",]$p.value))`; @tbl-abundModels).
...
#| echo: FALSE
#| message: FALSE
#| tbl-cap: "Coefficients of terms in total abundance GLMMs."
...
Mean total richness in winter (`r format(nStats[2,]$meanN, digits = 2)` ±`r format(nStats[2,]$seN, digits = 2)`) was consistently lower for each estuary than in the summer (`r format(nStats[1,]$meanN, digits = 2)` ±`r format(nStats[1,]$seN, digits = 2)`; [@fig-NPlot]). The global richness GLMMs had no significant time:system interaction terms for either season, so I dropped the interaction term from the final models (*p* = `r scales::pvalue(min(richFullInteraction$p.value))` - `r scales::pvalue(max(richFullInteraction$p.value))`). Total richness did not change during either summer or winter (summer *p* = `r scales::pvalue(richFinalTidy[richFinalTidy$term=="smallYear"&richFinalTidy$Season=="Summer",]$p.value)` and winter *p* = `r scales::pvalue(richFinalTidy[richFinalTidy$term=="smallYear"&richFinalTidy$Season=="Winter",]$p.value)`; @tbl-richGLMMs). Outputs of GLMMs revealed summer abundances were not related to water temperature (*p* = `r scales::pvalue(min(richFinalTidy[richFinalTidy$term=="temp_Z"&richFinalTidy$Season=="Summer",]$p.value))` - `r scales::pvalue(max(richFinalTidy[richFinalTidy$term=="temp_Z"&richFinalTidy$Season=="Summer",]$p.value))`; @tbl-richGLMMs) and positively related to SAV coverage for all systems (*p* = `r scales::pvalue(min(richFinalTidy[richFinalTidy$term=="bvc_Z"&richFinalTidy$Season=="Summer",]$p.value))` - `r scales::pvalue(max(richFinalTidy[richFinalTidy$term=="bvc_Z"&richFinalTidy$Season=="Summer",]$p.value))`; @tbl-richGLMMs). In contrast, winter total abundance was positively related to water temperature for all systems (*p* = `r scales::pvalue(min(richFinalTidy[richFinalTidy$term=="temp_Z"&richFinalTidy$Season=="Winter",]$p.value))` - `r scales::pvalue(max(richFinalTidy[richFinalTidy$term=="temp_Z"&richFinalTidy$Season=="Winter",]$p.value))`; @tbl-richGLMMs) but, as during summer, positively related to SAV coverage for all systems (*p* = `r scales::pvalue(min(richFinalTidy[richFinalTidy$term=="bvc_Z"&richFinalTidy$Season=="Summer",]$p.value))` - `r scales::pvalue(max(richFinalTidy[richFinalTidy$term=="bvc_Z"&richFinalTidy$Season=="Summer",]$p.value))`; @tbl-richGLMMs).
...
#| echo: FALSE
#| message: FALSE
#| tbl-cap: "ANCOVA model outputs for betadiversity indices over time. Bold values are significant at alpha 0.95."

betaTrendOut <- as.data.frame(betaSummerSummary) %>%
  full_join(as.data.frame(betaWinterSummary)) %>%
