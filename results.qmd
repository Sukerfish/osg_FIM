...
Results from post-hoc tests on total richness GLMMs indicated modeled average richness of Apalachicola Bay in summer was greater than all other estuaries, with no significant difference in means among the other three (*t* ≥ `r format(abs(as.data.frame(richMeanS$contrasts)$t.ratio[1]), digits = 3)` for all *p* \< 0.05 [@tbl-richModels]). In winter, modeled average richness was more nuanced ([@tbl-richModels-4]). Further, results of the GLMMs indicated that total richness was positively correlated with SAV for all systems and seasons. As with total abundance, water temperature was not significant in any estuaries during the summer but was again positively associated with total richness in all estuaries in winter ([@tbl-richModels]). Further, total richness had no discernible trends over time in any of the estuaries or seasons ([@tbl-richSyn]).

```{r}
#| label: tbl-richSyn
#| echo: FALSE
#| tbl-cap: "Summary of observed total richness response trends per system and season from total abundance model. Trend direction calculated using GLMM framework and significant at alpha 0.95."

richTab <- data.frame(
  Parameter = c("Year", "Year", "Water temperature", "Water temperature", "SAV coverage", "SAV coverage"), 
  Effect = c("No change", "No change", "No change", "Positive ", "Positive", "Positive"), 
  Season = c("Summer", "Winter", "Summer", "Winter", "Summer", "Winter"))
