...
Nestedness and turnover components of Bray-Curtis pairwise dissimilarity over time reveal a general trend of stability during the summer. However, during winter, particularly in the northern estuaries (AP and CK), there is considerably higher variability in the years after 2010 ([@fig-betadiversity]).
...
#| fig-width: 7.5
#| fig-height: 5
#| fig-cap: "Partitioned betadiversity calculated pairwise with respect to the first year of available data. Balanced represents nestedness and gradient represents turnover."
plot(betaTimePlot)
```
...
The relationship of temperature to richness and abundance was better defined when including environmental parameters. The formula used for all models was: `response ~ (1|year_Z) + temp_Z +  bvc_Z + offset(log(n_hauls)) + ar1(yearMonth + 0|systemZone)` where `response` was either total abundance or total richness per haul.
...
#| fig-width: 7.5
#| fig-height: 5
#| fig-cap: "Temperature averaged annually and plotted over time. Blue line is linear model fitted to each dataset."
plot(annualTemp)
```
...
NMDS reveals distinct separation among northern and southern estuaries in multivariate species-space. Using an `envfit` model, water temperature and BVC are best aligned with the distance between the two groups of estuaries ([@fig-NMDS_all]).
...
#| fig-width: 7.5
#| fig-height: 5
#| fig-cap: "NMDS plots based on annual average abundance (transformed) data."
#| fig-subcap: 
#|   - "Summer"
#|   - "Winter"
plot(summerNMDS)
...
In order to test whether this was significant, I followed up with a dbRDA. For all systems and seasons, water temperature and BVC indeed tested as significant ([@fig-RDAforAll]).
...
Then we look at the groups of each taxa ([@fig-sigSlopes_grouped]).
...
#| fig-width: 7.5
#| fig-height: 5
#| fig-cap: "Sig slopes groups plot"
