...
Mean total richness in winter (`r format(nStats[2,]$meanN, digits = 2)` ±`r format(nStats[2,]$seN, digits = 2)`) was consistently lower for each estuary than in the summer (`r format(nStats[1,]$meanN, digits = 2)` ±`r format(nStats[1,]$seN, digits = 2)`; [@fig-NPlot]). The initial richness GLMMs for each season had no significant Year:System interaction terms, therefore, I dropped this interaction term from the final models (*p* = `r scales::pvalue(min(richFullInteraction$p.value))` - `r scales::pvalue(max(richFullInteraction$p.value))`). The revised GLMMs revealed that total richness did not change during either summer or winter (summer *p* = `r scales::pvalue(richFinalTidy[richFinalTidy$term=="smallYear"&richFinalTidy$Season=="Summer",]$p.value)` and winter *p* = `r scales::pvalue(richFinalTidy[richFinalTidy$term=="smallYear"&richFinalTidy$Season=="Winter",]$p.value)`; @tbl-richGLMMs). Further, total richness in summer was not related to water temperature (*p* = `r scales::pvalue(min(richFinalTidy[richFinalTidy$term=="temp_Z"&richFinalTidy$Season=="Summer",]$p.value))`; @tbl-richGLMMs) and was positively related to SAV coverage for all systems (*p* = `r scales::pvalue(min(richFinalTidy[richFinalTidy$term=="bvc_Z"&richFinalTidy$Season=="Summer",]$p.value))`; @tbl-richGLMMs). In contrast, total richness in winter was positively related to water temperature for all systems (*p* = `r scales::pvalue(min(richFinalTidy[richFinalTidy$term=="temp_Z"&richFinalTidy$Season=="Winter",]$p.value))`; @tbl-richGLMMs). However, as during summer, total richness was positively related to SAV coverage for all systems (*p* = `r scales::pvalue(min(richFinalTidy[richFinalTidy$term=="bvc_Z"&richFinalTidy$Season=="Summer",]$p.value))`; @tbl-richGLMMs).
...
#| echo: FALSE
#| message: FALSE
#| tbl-cap: "Summary outputs from PERMANOVAs conducted for each season. Values in bold are significant at alpha 0.95."
summerdf <- as.data.frame(summerfAbundPERM) %>%
  mutate(Parameter = row.names(.)) %>%
  mutate(Season = "Summer") %>%
  rename(Pval = "Pr(>F)")

winterdf <- as.data.frame(winterfAbundPERM) %>%
  mutate(Parameter = row.names(.)) %>%
  mutate(Season = "Winter") %>%
  rename(Pval = "Pr(>F)")
...
perms <- summerdf %>%
  ...
  relocate(Parameter, .before = Df) %>%
  flextable(.) %>%
  merge_v(j = ~ Season) %>%
  colformat_double(j = c(4:7), digits = 3) %>%
  set_formatter(Pval = function(x) scales::pvalue(x)) %>%
  set_header_labels(Df = "df", 
    SumOfSqs = "Sum of squares", 
    Pval = "p-value") %>%
  bold(~ Pval <0.05, c(3:7)) %>%
  paginate(init = TRUE, hdr_ftr = TRUE) %>%
  autofit() %>%
  theme_vanilla()
...
Plotting the taxonomic groups of each significant taxa reveals the magnitude of change in boney fishes is considerably greater than in all other groups ([@fig-sigSlopes_grouped]). DROP DROP DROP
