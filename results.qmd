...
stuff <- dust(summerSummary)

# tempSummary <- as.data.frame(redOut) %>%
#   filter(systemSeason %in% c("AP_winter", "TB_winter", "CH_winter")) %>%
...
Water temperature for each system was significantly different in the winter, but not in the summer.
...
Seagrass coverage over time significantly changed in Cedar Key summer (negatively), Tampa Bay winter (positively), and in both seasons in Charlotte Harbor (positively) ([@fig-BVCPlot]; [@tbl-bvcModels]).
...
#| echo: FALSE
#| message: FALSE
#| tbl-cap: "Output of summer water temeprature linear model."
