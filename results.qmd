...
Throughout the study period, water temperature was increasing during winter for all systems except Cedar Key, with a range of increases from `r format(min(tempSummary$estimate_cYear), digits = 2)`°Cyr^-1^ ± `r format(max(tempSummary[2,]$std.error_cYear), digits = 2)` to `r format(max(tempSummary$estimate_cYear), digits = 2)`°Cyr^-1^ ± `r format(max(tempSummary[1,]$std.error_cYear), digits = 2)`. However, summer water temperatures were fairly stable during the 22-year period, with no statistically significant changes detected ([@fig-TempPlot]; [@tbl-tempModels]). Simultaneously, bottom vegetation coverage (BVC) increased through time in Charlotte Harbor in both seasons and in winter in Tampa Bay, but decreased during the summer in Cedar Key. In species-space, `r length(unique(sigSlopes$Scientificname))` taxa were changing across all systems out of a total `r length(unique(SlopesForAllDF$Scientificname))` identified taxa.
