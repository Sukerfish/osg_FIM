...
tempMeanOut <- as.data.frame(summary(summerSummaryAll$lsmeans)) %>%
  mutate(Season = "Summer", .before = system) %>%
  full_join(as.data.frame(summary(winterSummaryAll$lsmeans))) %>%
...
#| echo: FALSE 
#| message: FALSE 
#| tbl-cap: "Outputs of temperature ANCOVA models." 

# pander(summerSummary)
# pander(winterSummary)

tempANCOVAft <- summerSummary %>%
  mutate(Season = "Summer", .before = term) %>%
  full_join(winterSummary) %>%
  mutate(Season = ifelse(is.na(Season), "Winter", Season)) %>%
  flextable(.) %>%
  merge_v(j = ~ Season) %>%
...
(seasonTempPlot <- ggplot(winterTemp,
    ...
    geom_smooth(method = "lm", 
                formula = y ~ x,
                se = FALSE,
                data = winterTemp[winterTemp$system %in% c("AB", "TB", "CH"),],
                linetype = "solid") +
    geom_smooth(method = "lm", 
                formula = y ~ x,
                se = FALSE,
    ...
    geom_smooth(method = "lm", 
                formula = y ~ x,
                se = FALSE,
                data = summerTemp[summerTemp$system %in% c("AB"),],
                linetype = "solid") +
