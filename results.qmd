...
I conducted all analyses using the R Statistical Computing Environment v4.3.0 with an alpha of 0.05 for all significance testing [@R2023]. To address if water temperature and SAV have changed over time in eGOM estuaries, I used Analysis of Covariance (ANCOVA) models for summer and winter seasons, independently, with year as a continuous predictor and estuarine system as a categorical predictor (fixed, 4 levels: Apalachicola Bay, Cedar Key, Tampa Bay, Charlotte Harbor) plus their interaction. Each model included either the mean annual water temperature or percent SAV coverage for each season as responses. Prior to this analysis, we removed any outlier data from years in which temperatures were anomalously higher or lower than the long-term average (e.g. winter of 2010; [@stith2012]). I performed post-hoc contrasts for any significant terms in these ANCOVA models to determine the magnitude of any specific differences in means across systems using the *emmeans* package and used the Tukey method for *p*-value correction [@emmeans]. For any ANCOVA models with a significant interaction between year and system, I separated the model into independent linear models for each system and season combination. If the interaction between year and system was insignificant, I then dropped the interaction term and reran the model without it.

To address if the richness and abundance of fishes within eGOM estuarine faunal assemblages has changed over the study period and assess the effects of water temperature on these assemblages, I modeled total richness and total abundance per haul across all estuaries for both the summer and winter seasons using generalized linear mixed models (GLMMs) from the R package *glmmTMB* [@glmmTMB]. Given the well-documented seasonal differences in estuarine systems [see @Hagan2003; @Strydom2015; @Schrandt2020], in this and all subsequent analyses, I separately analyzed summer and winter. For each seine haul, I calculated the richness as the number of taxa present and total abundance as the sum of all individuals counted. For each model, the response in the GLMM included either the total richness or transformed total abundance of each haul, with estuary (fixed, 4 levels: Apalachicola Bay, Cedar Key, Tampa Bay, Charlotte Harbor), year (fixed, continuous; including the interaction), water temperature (fixed, continuous), and percent SAV coverage (fixed, continuous) as predictors. I applied z-score transformation to water temperature and percent SAV coverage to enable relative effect sizes comparisons. A covariance matrix was included in each model to account for serial autocorrelation and was calculated from month of sampling event and within-estuary zone. Given that species richness is strongly affected by sampling effort [@Mittelbach2019], I included an offset term to account for variation in the number of seine hauls. Thus, the formula used for each was: `response ~ year * system + temp_Z + sav_Z + offset(log(n_hauls)) + ar1(yearMonth + 0|systemZone)` where `response` was either total richness or total abundance per haul. The package *emmeans* was again used for post-hoc contrasts of means and the Tukey method was used for *p*-value correction [@emmeans]. For any GLMMs with a significant interaction between year and system, I separated the model into independent linear models for each system and season combination. If the interaction between year and system was insignificant, I then dropped the interaction term and reran the model without it.

To examine the composition and stability of eGOM estuarine fish assemblages over time, I started with a univariate approach using the R package *betapart* to calculate Bray-Curtis dissimilarity over time with respect to the first year of available data and modeled this with ANCOVA using the formula `beta diversity index ~ year + system + year:system` [@betapart]. For any ANCOVAs with a significant interaction between year and system, I separated the model into independent linear models for each system and season combination. If the interaction between year and system was insignificant, I dropped the interaction term and reran the model without it. I then used permutational multivariate analysis of variance (PERMANOVA) from the R package *vegan to* evaluate the interactive effect of estuary (fixed, four levels) and year (fixed, continuous) within season on the fourth-root transformed taxon abundances [@vegan]. I then followed up with canonical analysis of principal coordinates (CAP) ordinations using the R package *BiodiversityR* to visualize differences among the levels of any fixed effect categorical terms deemed significant by the PERMANOVA models (*sensu* [@Peake2022]; [@BiodiversityR]). In addition, I included taxon biplot correlation vectors in these CAP ordinations to better visualize which taxa contributed to any observed differences. Vector lengths were calculated by applying Pythagorean theorem to the first two axis scores for each taxon. To explore the effects of water temperature and SAV coverage on the relative abundances of the different taxa, I performed distance-based redundancy analysis (dbRDA) using the R package *vegan* [@vegan]. In all calculations, I used fourth-root transformation to reduce the effects of overly abundant taxa and square-root transformed Bray-Curtis dissimilarity matrices to eliminate negative eigenvalues [@Legendre1999].

Finally, to assess the balance of individual taxa increasing and decreasing in abundance over time I used a univariate approach following Dornelas et al. [-@Dornelas2019]. Within each estuary and season, I ran a linear model on the fourth-root transformed abundance of each taxon independently over time to determine both the magnitude and direction of change at the taxa level. I binned these rates of change to construct histograms displaying the distribution and magnitude of taxa rates of change. I followed up on these analyses to examine whether there were patterns in the climatology of any taxa that underwent significant change. I pulled climatological association data from Fishbase using *rfishbase* and created plots of the distributions of climatological association groups [@rfishbase]. Unless otherwise indicated, values from all analyses were reported as mean ± SE and plots were generated with packages *ggplot2* and *patchwork* [@ggplot2; @patchwork].

In total, 17,492 sampling events occurred where at least one taxon was caught and counted during summer and winter in any of the four estuaries. For seven sampling events, water temperature data were unavailable and thus excluded from the analyses. Further, hauls that were composed solely of rare species (taxa found in ≤5% of all samples for that system and season) were removed from the analyses which reduced the sample count down to `r format(nrow(SXS_filtered), big.mark=",")`. Overall, `r ncol(SXS_filtered_spp)` taxa were recorded in the dataset over `r max(as.numeric(as.character(waterBVC_full$seasonYear)))-min(as.numeric(as.character(waterBVC_full$seasonYear)))+1` years of collections.
...
#| fig-width: 6.5 
#| fig-height: 6 
#| fig-cap: "Annually averaged SAV coverage over time. Error bars represent ± 1 standard error. Solid lines indicate significant modeled trends and dashed lines indicate a trend of no significance and are derived from independent linear models."
...
#| echo: FALSE
#| message: FALSE
#| tbl-cap: "Coefficients of terms in total abundance GLMMs. Bold rows highlight terms where *p* < 0.05."
...
Mean total richness in winter (`r format(nStats[2,]$meanN, digits = 2)` ±`r format(nStats[2,]$seN, digits = 2)`) was consistently lower for each estuary than in the summer (`r format(nStats[1,]$meanN, digits = 2)` ±`r format(nStats[1,]$seN, digits = 2)`; [@fig-NPlot]). The initial richness GLMMs for each season had no significant Year:System interaction term in either model, therefore, I dropped this interaction term from the final models (*p* = `r scales::pvalue(min(richFullInteraction$p.value))` - `r scales::pvalue(max(richFullInteraction$p.value))`). The revised GLMMs revealed that total richness did not change during either summer or winter (summer *p* = `r scales::pvalue(richFinalTidy[richFinalTidy$term=="smallYear"&richFinalTidy$Season=="Summer",]$p.value)` and winter *p* = `r scales::pvalue(richFinalTidy[richFinalTidy$term=="smallYear"&richFinalTidy$Season=="Winter",]$p.value)`; @tbl-richGLMMs). Further, total richness in summer was not related to water temperature (*p* = `r scales::pvalue(min(richFinalTidy[richFinalTidy$term=="temp_Z"&richFinalTidy$Season=="Summer",]$p.value))`; @tbl-richGLMMs) and was positively related to SAV coverage for all systems (*p* = `r scales::pvalue(min(richFinalTidy[richFinalTidy$term=="bvc_Z"&richFinalTidy$Season=="Summer",]$p.value))`; @tbl-richGLMMs). In contrast, total richness in winter was positively related to water temperature for all systems (*p* = `r scales::pvalue(min(richFinalTidy[richFinalTidy$term=="temp_Z"&richFinalTidy$Season=="Winter",]$p.value))`; @tbl-richGLMMs). However, as during summer, total richness was positively related to SAV coverage for all systems (*p* = `r scales::pvalue(min(richFinalTidy[richFinalTidy$term=="bvc_Z"&richFinalTidy$Season=="Summer",]$p.value))`; @tbl-richGLMMs).
...
#| echo: FALSE
#| message: FALSE
#| tbl-cap: "Coefficients of terms in total richness GLMMs. Bold rows highlight terms where *p* < 0.05."
...
betaTrendOut <- as.data.frame(betaSummerSummaryANC_final) %>%
  ...
  flextable(.) %>%
  merge_v(j = ~ Season) %>%
  colformat_double(j = c(3:6), digits = 3) %>%
  # set_header_labels(system = "System", 
...
PERMANOVAs indicated assemblage composition and abundance stability varied differently over time among systems due to a significant interactive effect between year and system (Year:System interaction term for summer *p* = `r scales::pvalue(summerdf$p.value[3])` and winter *p* = `r scales::pvalue(winterdf$p.value[3])`; [@tbl-fullPERMANOVA]). Therefore, separate CAP ordinations were conducted for each system within each season (*sensu* @Peake2022).

```{r}
#| label: tbl-fullPERMANOVA
#| echo: FALSE
#| message: FALSE
#| tbl-cap: "Summary outputs from PERMANOVAs conducted for each season. Values in bold are significant at alpha 0.05."
...
The majority of taxa did not exhibit significant changes in abundance over time, with a large group centered around zero in both summer and winter ([@fig-winnersLosers]). There was no difference in overall percentage of taxa changing between systems or seasons (*p* = `r scales::pvalue(sigSig$p.value)` - `r scales::pvalue(sigSigSys_tidy[1,]$p.value)`). However, the percentage of taxa changing negatively was greater in the summer than the winter (*t* = `r format(negSig$statistic, digits = 3)`, *p* = `r scales::pvalue(negSig$p.value)`; [@tbl-sigTaxa]).
...
#| message: FALSE
#| warning: FALSE
#| tbl-cap: "Percent of observed taxa with significant changes in abundance over time as well as the split between positive and negative change within."
...
sigFT <- flextable(sigSumPrep) %>%
  ...
  set_formatter(sigPercent = function(x) sprintf("%.2f%%", x*100),
                posPercent = function(x) sprintf("%.2f%%", x*100),
                negPercent = function(x) sprintf("%.2f%%", x*100)
               ) %>%
...
The majority of these taxa were either tropical/subtropical-associated or tropical/subtropical/temperate-associated and still experienced rather modest changes in abundance, while a few taxa had rates of change an order of magnitude greater than the majority ([@tbl-ecoSlopes]; [@fig-sigSlopes]).
...
sigSlopesEcoC <- sigSlopes %>%
  ...
  mutate(tempLogic = fct_recode(tempLogic,
                                   ...
                                   'Tropical/Subtropical/Temperate' = "tropical/subtropical/temperate",
                                   'Subtropical' = "subtropical",
                                   'Subtropical/Temperate' = "subtropical/temperate",
...
#| message: FALSE
#| warning: FALSE
#| tbl-cap: "Climatological associations of taxa that significantly changed. Percentages listed are calculated with respect to number of taxa with available climate data."
...
ecoFT <- ecoTable %>%
  ...
  labelizor(part = "header", 
  labels = stringr::str_to_title) %>%
  set_formatter(posPercent = function(x) sprintf("%.2f%%", x*100),
                negPercent = function(x) sprintf("%.2f%%", x*100)
               ) %>%
  set_header_labels(count = "Taxa counted",
...
#| fig-width: 6.5
#| fig-height: 7.25
#| fig-cap: "Slopes of taxa abundance over time plotted per estuary and season. Red indicates a negative change over time and blue indicates a positive change. The dashed line represents a slope of 0. Error bars represent ± 1 standard error. Numbers on *y*-axis correspond to individual taxa, see [@tbl-ssKey] for key."
...
for (i in seasysKey){
  ...
  sigPlotsFree[[i]] <- ggplot(gf,
         aes(y = reorder(Scientificnumber, raw_slope),
             x = raw_slope)) + 
...
Water temperature increase in winter in line with global rate. Summer not so much. Maybe that is a Florida thing? From previous studies, we know that estuarine warming is a global phenomenon with rates ranging from 0.015 to 0.2°C yr^−1^ (Preston [2004](https://aslopubs.onlinelibrary.wiley.com/doi/full/10.1002/lno.12057#lno12057-bib-0055); Seekell and Pace [2011](https://aslopubs.onlinelibrary.wiley.com/doi/full/10.1002/lno.12057#lno12057-bib-0058); Fulweiler et al. [2015](https://aslopubs.onlinelibrary.wiley.com/doi/full/10.1002/lno.12057#lno12057-bib-0029); Scanes et al. [2020](https://aslopubs.onlinelibrary.wiley.com/doi/full/10.1002/lno.12057#lno12057-bib-0057)). Per [@bashevkin2022].
...
CAP ordinations suggest most change happens gradually. Cedar Key again shows up as an outlier... and has a distinct separation between groups of years. Similar in RDA, but difference is in spread of points (CK is broad).

Loss of Spot over time is interesting. Spot plot? Larval supply issues? Lots more to explore beyond scope of the project.

Other taxa changes could be more specific. Need to look for commonalities.
