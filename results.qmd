...
Total abundance was generally generally higher (average total abundance `r format(abundStats[1,]$meanAb, digits = 2)` ±`r format(abundStats[1,]$seAb, digits = 2)`) for all systems in the summer season compared to winter (average total abundance `r format(abundStats[2,]$meanAb, digits = 2)` ±`r format(abundStats[2,]$seAb, digits = 2)`; [@fig-AbundPlot]).
...
Results from post-hoc tests on total abundance GLMMs revealed that in summer, there were differences in mean total abundance between northern and southern systems, a positive correlation with SAV coverage, and no significant correlation with water temperature ([@tbl-abundModels]; [@tbl-abundSyn]). Only Tampa Bay exhibited any change in total abundance over time during the summer ([@tbl-abundSyn]). In contrast, there were more nuanced differences in total abundance among estuaries in the winter. For all systems, total abundance was positively correlated with with SAV coverage and water temperature. Cedar Key had the lowest total abundance compared to Apalachicola and Charlotte Harbor (*t* ≥ `r format(as.data.frame(abundMeanW$contrasts)$t.ratio[5], digits = 3)` for all *p* \< 0.05; contrast with Tampa Bay was marginally significant *t* = `r format(as.data.frame(abundMeanW$contrasts)$t.ratio[3], digits = 3)`, *p* = `r format(as.data.frame(abundMeanW$contrasts)$p.value[3], digits = 3)`; [@tbl-abundModels]; see also [@tbl-abundModelsRaw] for full output). In addition, total abundance in Apalachicola Bay decreased during the winter while all other estuaries exhibited no significant change over time (*t* ≥ `r format(as.data.frame(abundContW$contrasts)$t.ratio[4], digits = 3)` for all *p* \< 0.05 [@tbl-abundSyn]).
...
abMeanOut <- as.data.frame(summary(abundMeanS$emmeans)) %>%
  mutate(Season = "Summer", .before = system) %>%
...
Plotting the taxonomic groups of each significant taxa reveals the magnitude of change in boney fishes is considerably greater than in all other groups ([@fig-sigSlopes_grouped]).
