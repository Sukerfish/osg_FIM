...
bvcMeanOut <- as.data.frame(summary(summerSummaryBVCAll$lsmeans)) %>%
  ...
  flextable(.) %>%
  merge_v(j = ~ Season) %>%
  colformat_double(j = c("lsmean"), digits = 1) %>%
...
#| echo: FALSE
#| message: FALSE
#| tbl-cap: "Coefficients of variation in the Bray-Curtis dissimilarity index for each season and system."
...
#| label: PERMANOVA
#| include: FALSE
load("bigPERMANOVAs_yxs_fars.RData")
...
#| echo: FALSE
#| message: FALSE
#| tbl-cap: "Outputs from assemblage composition and abundance PERMANOVAs conducted for each system and season combination. Values in bold are statistically significant at alpha 0.05."
